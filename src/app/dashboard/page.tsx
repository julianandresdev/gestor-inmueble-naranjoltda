...
async function DashboardTareasPrioritarias() {
  const data = await getDashboardData();
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Vencidas, urgentes, importantes o sin asignar. Pendientes de cierre.
        </CardDescription>
      </CardHeader>
      <CardContent>
...
async function DashboardActividad() {
  const actividad = await listarActividadReciente(10);
  return (
    <Card>
      <CardHeader>
...
export default async function DashboardPage() {
  ...
  return (
    <main className="mx-auto flex min-h-[calc(100vh-3.5rem)] w-full max-w-6xl flex-col gap-8 px-4 py-10">
      ...
      <Suspense fallback={<KpiSkeleton cards={6} />}>
        <DashboardKpis />
      </Suspense>

      <section>
        <Suspense fallback={<CardSkeleton lines={6} />}>
          <DashboardTareasPrioritarias />
        </Suspense>
      </section>
